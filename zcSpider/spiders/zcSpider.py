...
class zcSpider(scrapy.Spider):
    ...
    def start_requests(self):
        start = datetime(2010, 1, 19)
        end = datetime(2010, 2, 18)
        #for day in arrow.Arrow.range('day', start, end):
        #    url = self.base_url + day.format('YYYY-MM-DD')
        #    yield Request(url, self.parse, meta={'year': day.format('YYYY')})
    ...
    def parse(self, response):
        ...
        print(response.body)
        datas = selector.xpath('//tr[@isend="1"]').extract()
        for data in datas:
            try:
                ...
                yield Request(url=url, callback=self.getYpData, meta={'mid': item['mid'], 'year': response.meta['year']})
                # print(item)
    ...
    def getOpData(self, response):
        ...
        try:
            ...
            for i in range(pageCount):
                ...
                yield Request(url=url, callback=self.parseOpData, meta={'mid': mid})
        except Exception as e:
    ...
    def parseOpData(self, response):
        ...
        for data in datas:
            try:
                ...
                print('[{0}]解析欧赔数据时发生错误{1}'.format(mid, e))
    ...
    def getYpData(self, response):
        ...
        try:
            ...
            for i in range(pageCount):
                ...
                yield Request(url=url, callback=self.parseYpData, meta={'mid': mid, 'year': year})
        except Exception as e:
            print('{0}解析博彩公司数量时发生错误{1}'.format(mid, e))
    ...
    def parseYpData(self, response):
        ...
        for data in datas:
            try:
                ...
                yield Request(url=url, callback=self.parseYpDetailData, meta={'mid': mid, 'lyName': item['mlyName']})
            except Exception as e:
                print('[{0}]解析亚赔数据时发生错误:{1}'.format(mid, e))
